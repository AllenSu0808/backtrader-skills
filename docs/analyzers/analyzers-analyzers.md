---
source: https://www.backtrader.com/docu/analyzers/analyzers/
fetched: 2026-07-17
title: "Analyzers"
---
...
The interface is modeled after that of _Lines_ objects, feature for example a `next` method but there is a major difference:
...
`Analyzer` objects are (like _strategies_, _observers_ and _datas_) added to the system through a `cerebro` instance:

*   `addanalyzer(ancls, *args, **kwargs)`

But when it comes to operation during `cerebro.run` the following will happen for each _strategy_ present in the system
...
*   If the backtesting run contains for example _3 strategies_ then _3 instances_ of `ancls` will be created and each of them will be attached to a different strategy.

Bottomline: _an analyzer analyzes the performance of a single strategy_ and _not the performance of an entires system_
...
These _sub-analyzers_ or _slave-analyzers_ will also be inserted into the same strategy as the one creating them. But they are completely invisible to the user.
...
*   `self.strategy`: reference to the strategy subclass in which the analyzer object is operating. Anything accessible by the _strategy_ can also be accessd by the _analyzer_
    
*   `self.datas[x]`: the array of data feeds present in the strategy. Although this could be accesed over the _strategy_ reference, the shortcut makes work more comfortable.
...
The _Analyzer_ base class creates a `self.rets` (of type `collections.OrderedDict`) member attribute to return the analysis. This is done in the method `create_analysis` which can be overriden by subclasses if creating custom analyzers.

## Modus operandi

Although `Analyzer` objects are not _Lines_ objects and therefore do not iterate over lines, they have been designed to follow the same operation pattern.
...
3.  `prenext` / `nextstart` / `next` will be invoked following the calculated minimum period of the _strategy_ the indicator is working in.
    ...
    It may be customary to call `len(self)` in _Lines_ objects to check the actual amount of bars. This also works in `Analyzers` by returning the value for `self.strategy`
...
Once the regular operations cycle has been completed, the _analyzers_ featuring additional methods for extracting/outputting information
...
*   `pprint` (_pretty print_) uses the Python `pprint` module to print the `get_analysis` resutls.
...
*   `get_analysis` creates a member attribute `self.ret` (of type `collections.OrderedDict`) to which analyzers write the analysis results.
    ...
    Subclasses of _Analyzer_ can override this method to change this behavior
    

## Analyzer Patterns

Development of _Analyzer_ objects in the `backtrader` platform have revealed 2 different usage patterns for the generation of the analysis:
...
2.  Gather (or not) the information as above, but generate the analysis in a single pass during the `stop` method
    ...
    The `SQN` (_System Quality Number_) gathers trade information during `notify_trade` but generates the statistic during the `stop` method
...
*   `params` declaration
    ...
    Although the declared ones are not used (meant as an example), _Analyzers_ like most other objects in `backtrader` support parameters
    
*   `__init__` method
    
    Just like _Strategies_ declare _Indicators_ in `__init__`, the same do analyzers with support objects.
...
*   `self.strategy` (giving access to the _strategy_ and anything accessible from it)
...
This is not a _Lines_ object, but the methods and operation follow the same design
...
The important thing is to override `get_analysis` to return a _dict-like_ object containing the results of the analysis (the actual format is implementation dependent)
...
#### notify\_cashvalue(cash, value)

Receives the cash/value notification before each next cycle

#### notify\_fund(cash, value, fundvalue, shares)
...
Returns a _dict-like_ object with the results of the analysis

The keys and format of analysis results in the dictionary is implementation dependent.

It is not even enforced that the result is a _dict-like object_, just the convention
...
Prints the results returned by `get_analysis` using the pretty print Python module (_pprint_)
