---
source: https://www.backtrader.com/docu/mixing-timeframes/indicators-mixing-timeframes/
fetched: 2026-07-17
title: "Indicators - Timeframe Mixing"
---

# Mixing Timeframes in Indicators

[Release 1.3.0.92](https://github.com/mementum/backtrader/releases/tag/1.3.0.92) brings up the possibility to have data (from either data feeds and/or indicators) from different timeframes mixed.

Background: _Indicators are smart dumb objects._
...
Here a _sell signal_ is sought when the close is below the `s1` line (_1st support_)
...
And for a good reason: `self.data.close` provides values from the very 1st instant, but `PivotPoint` (and hence the `s1` line) will only deliver values once a **full month** has gone by, which is roughly equivalent to 22 values of `self.data0.close`. During this 22 _closes_ there isn’t yet a value for `s1` and the attempt to fetch it from the underlying array fails.

_Lines_ objects support the `(ago)` operator (`__call__` special method in _Python_) for deliver a delayed version of itself:
...
In this mode the indicators and late-evaluated automatic _lines_ objects are executed step by step rather than in tight loops. This makes the entire operation slower, but it makes it **possible**
...
The script also provides insight into the additional possiblity: _couple all lines of an indicator_. Before we had:
...
[![image](../indicators-mixing-timeframes.png)](https://www.backtrader.com/docu/mixing-timeframes/indicators-mixing-timeframes.png)

## Full coupling syntax

For _lines_ objects with multiple lines (for example _Indicators_ like `PivotPoint`):
...
For _lines_ objects with a single line (for example line `s1` from the indicator `PivotPoint`):
