---
source: https://www.backtrader.com/docu/inddev/
fetched: 2026-07-17
title: "Indicators - Development"
---
...
*   Define the _lines_ it will hold
...
*   Provide a fully defined operation in `__init__` with a binding (assignment) to the line(s) of the indicator or else provide `next` and (optionally) `once` methods
    ...
    Optimization of the calculation for the **runonce** mode (batch operation) can be achieved by providing a _once_ method.
...
`bt.Max` returns an _lines_ object that is automatically iterated for each bar passed to the indicator.
...
Here is a potential implementation of a _Simple Moving Average_:
...
The sample _SimpleMovingAverage_ may be done on for example:

*   A regular data feed
    
    This has a default mininum period of 1 (just wait for the 1st bar that enters the system)
    
*   Another Moving Average … and this in turn already has a _period_
...
The `addminperiod` method is telling the system to take into account the extra _period_ bars needed by this indicator to whatever minimum period there may be in existence.
...
A quick _MACD_ implementation with Histogram:
...
*   `EMA` stands for _Exponential Moving Average_ (a platform built-in alias)
...
*   The named lines of the indicator “macd” and “signal” are being assigned objects which already carry declared (behind the scenes) periods
    ...
    *   signal takes directly the period of the Exponential Moving Average over macd. This EMA also takes into account the already existing macd period and the needed amount of samples (period\_signal) to calculate itself
...
Although more can be seen in the _Plotting_ section and to have a behaved and nice citizen in the plotting world, a couple of things can be added:
